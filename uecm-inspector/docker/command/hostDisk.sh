    ...
    spec:
      ...
      - name: $jobNamePrefix-$k3sNode
        ...
        args:
          ...
          - "-c"
          - >-
            df --output=used,pcent -h /var | sed 1d | xargs -I{} echo "\$(hostname) /var used {}" | sed 's/\s\+/ /g';
            df --output=used,pcent -h /mnt | sed 1d | xargs -I{} echo "\$(hostname) /mnt used {}" | sed 's/\s\+/ /g';
        securityContext:
          privileged: true
      nodeSelector:
